...
export default class Pending extends React.Component {
            ...
            if (this.state.userlevel == 1) {
                fetch(global.PATH, {
                    ...
                    body: JSON.stringify({
                        ...
                        uid: this.state.uid,
                        verifiedStatus: this.state.status
                    })
                }).then((response) => response.json())
                    .then((responseJson) => {
                        if (responseJson.result == "success") {
                            this.setState({ fetchLoading: false, limitExceed: false, data: [...this.state.data, ...responseJson.posts] });
                        } else {

                            this.setState({ fetchLoading: false, limitExceed: true });
                        }
                    }).catch((error) => {
                        ToastAndroid.show(
                            'Error Calling ',
    ...
    Directions = (latituded, longituded) => {
        const data = {
            ...
            destination: {
                latitude: parseInt(latituded),
                longitude: parseInt(longituded)
            },
            params: [
                {
                    key: "travelmode",
                    ...
                    <View style={styles.page}>
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <View style={{ flex: 0.6, paddingTop: 15, flexDirection: 'row' }}>
                                        ...
                                        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={[AppStyles.oddLMedium, { color: '#FFFFFF', letterSpacing: 0.5, paddingLeft: 10, paddingTop: 5 }]}>My Posts</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', padding: 15 }}>
                            <TouchableOpacity onPress={() => this.setState({ filter: !this.state.filter })}>
                                <Image source={require('../../../images/paper.png')} style={{ width: 30, height: 30, overflow: 'hidden' }} resizeMode='contain' />
                            </TouchableOpacity>
                        </View>
                        {
                            this.state.filter == 1 &&
                            <View style={{ padding: 10 }}>
                                <Text style={{ ...AppStyles.evenLBold }}>Product Category & Sub-Category</Text>
                                <MaterialDropdown
                                    ...
                                    errorColor={'#000000'}
                                    lineWidth={1.5}
                                    onChangeText={(value, label) => { this.setState({ category: value, limitExceed: false, data: [] }), this.GetSubcatg(), this.GetData() }}
                                />
                                <MaterialDropdown
                                    ...
                                    errorColor={'#000000'}
                                    ref={"subcategory"}
                                    onChangeText={(value) => { this.setState({ subcategory: value, limitExceed: false, data: [] }), this.GetData() }}
                                />
                                {this.state.userlevel == 1 &&
                                    <MaterialDropdown
                                        ...
                                        errorColor={'#000000'}
                                        ref={"subcategory"}
                                        onChangeText={(value) => { this.setState({ status: value, limitExceed: false, data: [] }), this.GetData() }}
                                    />
                                }
                            </View>
                        }
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', padding: 10 }}>
                            <View style={{ flex: 0.7 }}>
                                <Text style={[AppStyles.evenLBold, { color: "#FFA502" }]}>Total Posts</Text>
                            </View>
                            <View style={{ flex: 0.1 }}>
                                <Text style={[AppStyles.evenLBold, { color: "#FFA502" }]}>: </Text>
                            </View>
                            <View style={{ flex: 0.2 }}>
                                <Text style={[AppStyles.evenLBold, { color: "#000000" }]}>{this.state.data.length}</Text>
                            </View>
                        </View>
                        ...
                        <FlatList
                            ...
                            renderItem={
                                ({ item }) =>
                                    <TouchableOpacity onPress={() => { Actions.ViewPost({ data: item }) }}>
                                            ...
                                            <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'space-between', marginTop: 10 }}>
                                                <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 10 }}>
                                                    ...
                                                    <View style={{ flex: 0.4 }}>
                                                        ...
                                                        <Text style={[AppStyles.oddSRegular, { textAlign: 'left' }]}>{item.timestamp}</Text>
                                                    </View>
                                                </View>
                                            </View>
                                            {
